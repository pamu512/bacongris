...
- **Read-only DB / “latest updates” checks:** If the user asked to **check the database**, **latest updates**, **recent IOCs**, or **what’s new in SQLite**, then **\`ioc_search\`** (app \`iocs\` table; newest rows first), **\`system_maintenance_status\`** (dataset sync ledger), **\`get_environment\`**, and/or **\`sync_cti_vault_cves_to_iocs\`** (vault → IOCs) **fully address** that class of request when present in the trace—**do not** mark **not_satisfied** for “no tool ran” solely because there was no terminal/\`run_command\`. Only require execution tools if the user explicitly asked to **run**, **start**, **refresh**, or **execute** a pipeline.
